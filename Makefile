...
FA_IMAGE := kailash-classification-fastapi
FA_CONTAINER := ${USER}-fastapi-classification
FA_PORT := 8502
...
build-image:
	@cd devops && \
		...
		FA_IMAGE=$(FA_IMAGE) FA_CONTAINER=$(FA_CONTAINER) FA_PORT=$(FA_PORT) \
...
start-container:
	...
	@docker ps --format '{{.Names}}' | grep -q "^$(MF_CONTAINER)$$" && echo "Already running container: \033[1;32m$(MF_CONTAINER)\033[0m" || true
	@docker ps --format '{{.Names}}' | grep -q "^$(FA_CONTAINER)$$" && echo "Already running container: \033[1;32m$(FA_CONTAINER)\033[0m" || true
	...
	@if ! docker ps --format '{{.Names}}' | grep -q -e "^$(JP_CONTAINER)$$" -e "^$(MF_CONTAINER)$$" -e "^$(SL_CONTAINER)$$"; then \
		...
		echo "Successfully started container: \033[1;32m$(FA_CONTAINER)\033[0m"; \
		FA_URL="https://0.0.0.0:$(FA_PORT)"; echo "FastAPI is running at: \033[1;34m$$FA_URL\033[0m"; \
...
enter-fastapi-container:
	@echo "You are inside the Container: \033[1;33m$(FA_CONTAINER)\033[0m"
	@docker exec -u root -it $(FA_CONTAINER) bash || true
...
stop-container:
	@if docker ps -a --format '{{.Names}}' | grep -q -e "^$(JP_CONTAINER)$$" -e "^$(MF_CONTAINER)$$" -e "^$(SL_CONTAINER)$$"; then \
		...
		echo "Stopped and removed container: \033[1;31m$(FA_CONTAINER)\033[0m"; \
		docker rm -f $(FA_CONTAINER) > /dev/null 2>&1; \
